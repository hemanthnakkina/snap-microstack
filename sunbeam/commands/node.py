...
@click.command()
@click.option(
    "--name", type=str, help="Fully qualified node name"
)
@click.option(
    "--role", default="converged", type=str, help="Role of the node"
)
def add_node(name: str, role: str) -> None:
    ...
    if step.is_skip():
        ...
        click.echo("Node already part of the microstack cluster")
    else:
...
@click.command()
@click.option(
    "--token", type=str, help="Join token"
)
